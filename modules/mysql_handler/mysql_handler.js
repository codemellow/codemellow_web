...
// exports.insert_new_repository = function(repository_name, user_name, repository_description, callback){

//   //TODO - parameter checking
//   if(typeof(callback) != 'function') callback = function(){};
//   var queryString = "";
//   queryString += "insert into cm_repository (repository_name, maintainer_id, repository_description) values (?, ";
//   queryString += "(select user_id from cm_user where user_name=?), ?);";

//   var parameter = [];
//   parameter.push(repository_name);
//   parameter.push(user_name);
//   parameter.push(repository_description);
...
exports.do_auth = function(repository_name, user_name, password, callback){
  ...
  var result = {}
  var queryString = "";
  queryString += "select * from cm_user where (user_id = ?) and (user_passwd = ?);";
...
exports.get_contributor_info  = function(params, callback){
 //TODO - parameter checking
  if(typeof(callback) != 'function') callback = function(data){return data};

  var qs = "";
  qs += "SELECT cm_contributor.*, cm_user.user_name FROM cm_contributor NATURAL JOIN cm_user "
  qs+=" WHERE repository_id=?";

  var params_arr = [];
  params_arr.push(params.repository_id);

  dbconn.query(qs, params_arr, function(err,rows,cols){

    if(err) {
      console.log('get_contributor_info error', err);//TODO - handle err.
      callback([])
    }
    else if(rows.length==0){
      callback([]);
...
exports.insert_contributor_info = function(params, callback){
  //TODO - parameter checking
  if(typeof(callback) != 'function') callback = function(data){return data};

  var qs = "";
  qs+="INSERT INTO cm_contributor (user_id, repository_id) "
  qs+="SELECT * FROM "
  qs+="(SELECT user_id FROM cm_user WHERE user_name=? LIMIT 1)  user, "
  qs+="(SELECT repository_id FROM cm_repository WHERE repository_name=? LIMIT 1) repo "

  var params_arr = [];
  params_arr.push(params.user_name);
  params_arr.push(params.repository_name);
  ...
  dbconn.query(qs, params_arr, function(err,data){
    if(!err && data && data.affectedRows > 0){
      ...
      console.log('insert_contributor_info err', err, data);
      callback(false);
    }
  });
...
exports.update_contributor_info = function(params, callback){
  //TODO - parameter checking
  if(typeof(callback) != 'function') callback = function(data){return data};

  var qs = "";
  qs += "update cm_contributor SET ?? = ? where repository_id=(select repository_id from cm_repository where repository_name=? LIMIT 1) AND user_id=? ";

  var params_arr = [];
  params_arr.push(params.update_key);
  params_arr.push(params.update_value);
  params_arr.push(params.repository_name);
  ...
  dbconn.query(qs, params_arr, function(err,data){
    if(!err && data && data.affectedRows > 0){
      ...
      console.log('update_contributor_info err', err, data);
      callback(false);
    }
  });
...
exports.is_maintainer = function(params, callback){
     //TODO - parameter checking
  if(typeof(callback) != 'function') callback = function(data){return data};

  var qs = "";
  qs += "SELECT * FROM cm_repository where repository_id=(select repository_id from cm_repository where repository_name=? LIMIT 1) and maintainer_id=? LIMIT 1";  
  var params_arr = [];
  params_arr.push(params.repository_name);
  params_arr.push(params.user_id);
  dbconn.query(qs, params_arr, function(err,rows){
...
exports.get_contributing_level = function(params, callback){
   //TODO - parameter checking
  if(typeof(callback) != 'function') callback = function(data){return data};

  var qs = "";
  qs += "SELECT * FROM cm_contributor where repository_id=(select repository_id from cm_repository where repository_name=? LIMIT 1) and user_id=? LIMIT 1";
  ...
  dbconn.query(qs, params_arr, function(err,rows){
    if(!err && rows ){
      ...
      console.log('get_contributing_level err', err);
      callback(-2);
    }
  }); 
}
...
exports.get_user_project_list = function(user_id, callback){
  if(typeof(callback) !== 'function') callback = function(data){return data};
  var project_list_query = '';
  project_list_query += 'SELECT * from cm_repository where maintainer_id = ?';

  dbconn.query(project_list_query, user_id, function(err, project_list){
    if(err)
      return callback(err);
    else
      callback(false, project_list);
  });
}
exports.get_user_contributing_project_list = function(user_id, callback){
  if(typeof(callback) !== 'function') callback = function(data){return data}; 
  var contributing_project_list_query = '';
  contributing_project_list_query += 'SELECT * FROM cm_repository WHERE repository_id IN (SELECT repository_id from cm_contributor where user_id = ?)';

  dbconn.query(contributing_project_list_query, user_id, function(err, project_list){
    if(err)
      return callback(err);
    else
      callback(false, project_list);
...
exports.best_seller = function(params, user, callback){
  ...
  if(params && params.start && params.end){
    ...
    get_repository_info  ='SELECT P.repository_name FROM cm_repository P, (' + get_repository_id + ') C WHERE C.repository_id=P.repository_id'
    ...
    get_repository_info  ='SELECT P.repository_description, P.thumbnail_url, P.review_point, P.review_count, P.repository_name, P.repository_id, P.repository_like_count FROM cm_repository P, (' + get_repository_id + ') C WHERE C.repository_id=P.repository_id ORDER BY C.count'
    dbconn.query(get_repository_info, function(err, data){
      if(err){
        ...
        /*Is there any best seller result*/
        if(data.length == 0){
          exports.get_recent_added_project(function(result){
            data = result;
            if(!data){
              callback(false);
              return;
            ...
            if(user){
              //Check whether user like projects in the result or not.
...
exports.get_recent_added_project = function(callback){
  var get_repository_id, get_repository_info, language_id_query, language_query, user_name, get_like;
  get_repository_id    ='SELECT repository_id from cm_repository ORDER BY create_date DESC LIMIT 0, 15';
  get_repository_info  ='SELECT P.repository_description, P.thumbnail_url, P.review_point, P.review_count, P.repository_name, P.repository_id, P.repository_like_count FROM cm_repository P, (' + get_repository_id + ') C WHERE C.repository_id=P.repository_id'
  dbconn.query(get_repository_info, function(err, data){
    if(err){
      console.log('err', err);
      callback(false);
...
exports.get_user_by_user_name = function(user_name, callback){
  var queryString = "SELECT * FROM cm_user WHERE user_name=? LIMIT 1"
  var params = [];
...
exports.get_dev_info = function(user_id, callback){
  var queryString = ""
  queryString += "SELECT "
  queryString += "( SELECT COUNT(*) FROM cm_contributor WHERE user_id=? ) AS contributions, "
  queryString += "( SELECT COUNT(*) FROM cm_repository WHERE maintainer_id=? ) AS repositories, "
  queryString += "( SELECT COUNT(*) FROM cm_follow WHERE user_id=? ) AS followers, "
  queryString += "( SELECT COUNT(*) FROM cm_follow WHERE following_id=? ) AS following"
  var params = [];
  params.push(user_id)
  params.push(user_id)
  params.push(user_id)
...
exports.get_repository_info_by_id_arr = function(params, callback){
  var qs='SELECT * FROM cm_repository ';
  qs+="WHERE repository_id IN ("+params.repository_id_arr.join(',')+") "
  qs+=" ORDER BY FIELD (repository_id, "+params.repository_id_arr.join(',')+") "


  dbconn.query(qs, function(err, rows){
    if(err){
      console.log(err)
      callback([]);
    }else{
...
exports.autocomplete_user = function(params, callback){
  var qs="SELECT user_name FROM cm_user WHERE user_name like binary('[query]%')";
  dbconn.query(qs.replace('[query]',params.query), function(err, rows){
    if(err){
      console.log('autocomplete_user err', err);
      callback([]);
    }else{
      // var ret=[];
      // for(var i=0;i<rows.length;i++){
      //   ret.push(rows[i].user_name)
      // }
      callback(rows);
    }
  })
