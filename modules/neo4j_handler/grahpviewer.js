(function () {
    ...
    var L = {},
        k = {};
    var i, y;
    var r = d3.layout.tree().size([360, height2 / 2 - R]).separation(function (Y, X) {
    ...
    d3.json("/wp-content/themes/conversation_theme/metadata.php", function (X, Y) {
        ...
        q.forEach(function (aa) {
            aa.key = replace_speacial_char(aa.name);
        ...
        T.get("episodes").forEach(function (aa) {
            aa.links = aa.links.filter(function (ab) {
                return typeof x[replace_speacial_char(ab)] !== "undefined" && ab.indexOf("r-") !== 0
            });
            j.set(aa.key, aa.links.map(function (ab) {
                var ac = replace_speacial_char(ab);
    ...
    function mouseout_erase_effect_func() {
        k = {
            node: null,
            map: {}
        };
        perspective_remove_effect()
    }

    function mouseover_effect_func(X) {
        if (k.node === X) {
            return
        }
        k.node = X;
    ...
    function M() {
        ...
        }).transition().duration(w).ease(fontname).attr("d", function (X) {
            return v([
                [X.x1, X.y1],
    ...
    function e() {
        ...
        V();
        B.selectAll("path").transition().duration(w).ease(fontname).attr("d", W);
        D([]);
        b(X);
        C([L.node]);
        m([]);
    ...
    function b(X) {
        var X = E.selectAll(".node").data(X, get_key);
        ...
        }).attr("class", "node").on("mouseover", mouseover_effect_func).on("mouseout", mouseout_erase_effect_func).on("click", G);
        ...
        X.transition().duration(w).ease(fontname).attr("transform", function (Z) {
            if (Z === L.node) {
                return null
            }
        ...
        X.selectAll("circle").transition().duration(w).ease(fontname).attr("r", function (Z) {
            if (Z == L.node) {
                return 100
            } else {
                if (Z.isGroup) {
        ...
        X.selectAll("text").transition().duration(w).ease(fontname).attr("dy", ".3em").attr("font-size", function (Z) {
            if (Z.depth === 0) {
                return 20
            } else {
                return 15
    ...
    function V() {
        var X = B.selectAll("path").data(H, get_key);
    ...
    function C(Z) {
        var ac = d.selectAll(".detail").data(Z, get_key);
    ...
    function D(Y) {
        var Y = f.selectAll(".episode").data(Y, get_key);
        var X = Y.enter().append("g").attr("class", "episode").on("mouseover", mouseover_effect_func).on("mouseout", mouseout_erase_effect_func).on("click", G);
        X.append("rect").attr("x", U / -2).attr("y", K / -2).attr("width", U).attr("height", K).transition().duration(w).ease(fontname).attr("x", function (Z) {
            return Z.x
        }).attr("y", function (Z) {
            return Z.y
        });
        X.append("text").attr("x", function (Z) {
            ...
            return Z.name
        }).transition().duration(w).ease(fontname).attr("x", function (Z) {
            return Z.x + t
        }).attr("y", function (Z) {
            return Z.y + o
        });
        Y.exit().selectAll("rect").transition().duration(w).ease(fontname).attr("x", function (Z) {
            return U / -2
        }).attr("y", function (Z) {
            return K / -2
        });
        Y.exit().selectAll("text").transition().duration(w).ease(fontname).attr("x", function (Z) {
            return U / -2 + t
        }).attr("y", function (Z) {
            return K / -2 + o
        });
    ...
    function m(Y) {
        ...
        }).attr("stroke", "#000").attr("stroke-width", 1.5).transition().duration(w).ease(fontname).attr("d", function (Z) {
            return v([
                [Z.x1, Z.y1],
                [Z.xx, Z.yy],
                [Z.x2, Z.y2]
    ...
    function perspective_remove_effect() {
    ...
    function replace_speacial_char(X) {
